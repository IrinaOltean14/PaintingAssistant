...
def get_dataloaders(train_df, val_df, test_df, batch_size):
    ...
    train_dataset = SEMARTDataset(train_df, train_t)
    val_dataset = SEMARTDataset(val_df, test_val_transforms)
    test_dataset = SEMARTDataset(test_df, test_val_transforms)
...
def train_model(model, criterion_school, criterion_type, optimizer, scheduler, dataloaders, sizes, save_path, patience=7, epochs=25):
        ...
        for phase in ['train', 'val']:
            ...
            for inputs, school_labels, type_labels in tqdm(dataloaders[phase], desc=f"{phase} Phase"):
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    outputs_school, outputs_type = model(inputs)
...
if __name__ == "__main__":
    ...
    images_path = "../semart/Images"
    train_df, val_df, test_df, enc_sch, enc_typ = load_and_encode_data(
        '../semart/semart_train.csv',
        '../semart/semart_val.csv',
        '../semart/semart_test.csv',
        images_path
    )

    model = MTL(len(enc_sch.classes_), len(enc_typ.classes_)).to(device)
